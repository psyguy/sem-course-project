...
items.identif <- c("concentrat",
                   "dayno",
                   "beepno"
                   )

items.anxiety <- c("anxious",
                   "doubt",
                   "guilty",
                   "lonely",
                   "suspic",
...
colnames(data.tmp) <- gsub(".*_", "", colnames(data.tmp))


d <- data.orig %>% select(contains(items.identif), contains(items.noniden))
...
f <- make.factors(items = c(items.p, items.n),
                  factor.name = "g",
                  num.times = 1,
                  skip.first = 5)

d.l <- d %>% my.lagger(items = c(items.p, items.n), identifiers = identifiers)
