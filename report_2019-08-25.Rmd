...
l_o <- function(m, d = d){
  ...
  s <- f %>% parameterEstimates(add.attributes = TRUE)
  ...
  o <- list(f = f,
            s = s,
            l = s %>% filter(op == "=~") %>% select(-op, -exo),
            vcv = s %>% filter(op == "~~") %>% select(-op, -exo),
            r = s %>% filter(op == "~") %>% select(-op, -exo),
            inc = s %>% filter(op == "~1", est != 0) %>% select(-op, -exo),
            thr = s %>% filter(op == "|") %>% select(-op, -exo),
            ch = round(fm["chisq"],2),
            df = round(fm["df"],2),
            p = round(fm["df"],4),
            cfi = round(fm["cfi"],4),
...
This model had a decent fit ($\chi^2 =$ `r o2$ch`, $df =$ `r o2$df`, $RMSEA =$ `r o2$rmsea`, $CFI =$ `r o2$cfi`, $SRMR =$ `r o2$srmr`, and $TLI =$ `r o2$tli`.)


```{r tbl, results="asis", echo=FALSE, include=TRUE}
